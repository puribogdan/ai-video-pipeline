...
class FallbackLLMProvider(LLMProvider):
    """Fallback provider that tries Claude first, then OpenAI"""
    
    def __init__(self, claude_key: str, openai_key: str):
        self.primary = ClaudeProvider(claude_key)
    ...
    def chat_json(self, messages: List[Dict], **kwargs) -> Dict:
        ...
        try:
            print("[DEBUG] Attempting Claude (primary)...")
            result = self.primary.chat_json(messages, **kwargs)
            self.last_used_provider = "claude"
            print("[DEBUG] Successfully used Claude")
            return result
            
        except Exception as primary_error:
            print(f"[WARNING] Claude failed: {primary_error}")
            ...
            try:
                ...
                print("[DEBUG] Successfully used OpenAI as fallback")
                return result
                
            except Exception as fallback_error:
                print("[ERROR] Both Claude and OpenAI failed")
                print(f"[ERROR] Claude error: {primary_error}")
                print(f"[ERROR] OpenAI error: {fallback_error}")
